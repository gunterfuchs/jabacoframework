...
public class Label extends JLabel implements VB.IJabacoControl, MouseListener, MouseMotionListener, KeyListener, FocusListener {
   ...
   private void Class_Initialize() {
      ...
      try {
         $Parent().setVerticalAlignment(SwingConstants.TOP);
      } finally {
         return;
      }
   }
   ...
   // $VF: Could not inline inconsistent finally blocks
   // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
   public VB.fmTextAlign $TextAlign() {
      VB.fmTextAlign $TextAlign = new VB.fmTextAlign();
      Throwable Err = null;
      int h5 = 0;
      h5 = (boolean)0;

      try {
         h5 = $Parent().getHorizontalAlignment();
         if (h5 != SwingConstants.LEFT) {
            if (h5 != SwingConstants.CENTER) {
               if (h5 == SwingConstants.RIGHT) {
                  $TextAlign = VB.fmTextAlign.fmTextAlignRight;
               }
            } else {
               $TextAlign = VB.fmTextAlign.fmTextAlignCenter;
            }
         } else {
            $TextAlign = VB.fmTextAlign.fmTextAlignLeft;
         }
   ...
   // $VF: Could not inline inconsistent finally blocks
   // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
   public void $TextAlign(VB.fmTextAlign val) {
      Throwable Err = null;
      new VB.fmTextAlign();
      VB.fmTextAlign h5 = null;

      try {
         if (val.compareTo(VB.fmTextAlign.fmTextAlignLeft) != 0) {
            if (val.compareTo(VB.fmTextAlign.fmTextAlignCenter) != 0) {
               if (val.compareTo(VB.fmTextAlign.fmTextAlignRight) == 0) {
   ...
   private void SetMyBorder() {
      ...
      try {
         $Parent().setBorder(VB.Global.GetVBBorder($BorderStyle(), $SpecialEffect(), $BorderColor()));
   ...
   public void $Picture(IResource param1) {
      ...
      // 1b: aload 0
      // 1c: aload 1
      // 1d: putfield VB/Label.myPicturePath LVBA/IResource;
      // 20: aload 0
      // 21: invokevirtual VB/Label.$Parent ()Ljavax/swing/JLabel;
      // 24: aload 0
      // 25: getfield VB/Label.myPicturePath LVBA/IResource;
      // 28: invokestatic VB/Global.LoadIcon (LVBA/IResource;)Ljavax/swing/ImageIcon;
      // 2b: checkcast javax/swing/Icon
      // 2e: invokevirtual javax/swing/JLabel.setIcon (Ljavax/swing/Icon;)V
      // 31: goto 34
      // 34: nop
      // 35: nop
      // 36: nop
      ...
      // 38: nop
      // 39: aload 2
      // 3a: checkcast java/lang/Throwable
      // 3d: invokestatic VBA/Interaction.MsgBox (Ljava/lang/Throwable;)V
      // 40: goto 36
      // 43: astore 3
      // 44: astore 2
      // 45: ret 3
